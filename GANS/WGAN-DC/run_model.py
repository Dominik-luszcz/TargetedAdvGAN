...
def train_on_one_stocks(
    ...
    trainer = pl.Trainer(
        ...
        logger=False,
        callbacks=[train_callback, WGAN_DC_Callback(dataset.unscaled_returns)],
...
def plot_loss_functions(model: WGAN_DC, output_path):
...
def inference_test(model: WGAN_DC, noise_dim):
...
def sample_stats(model: WGAN_DC, log_returns, num_to_sample, output_dir):
