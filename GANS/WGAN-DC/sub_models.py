...
class Discriminator(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim):
        ...
        self.tcn1 = TCN_Block(
            input_dim=1, output_dim=64, kernel_size=3, dilation=1, padding=(3 - 1) * 1
        )
        self.gru1 = nn.GRU(
        ...
        self.tcn3 = TCN_Block(
            input_dim=64, output_dim=32, kernel_size=5, dilation=4, padding=(5 - 1) * 4
        )
        self.mlp = nn.Sequential(
            nn.Linear(64, 32), nn.LeakyReLU(negative_slope=0.2), nn.Linear(32, 1)
        )
