...
class DCGAN_Callback(pl.Callback):
    def __init__(self, log_returns, scaled_returns, days, initial_prices, num_to_sample):
        super().__init__()
    ...
    def on_train_epoch_end(self, trainer: pl.Trainer, model: AdversarialNetwork):
        ...
        for i in range(num_to_sample):
            ...
            f = self.scaled_returns[start + model.sample_size: start + model.sample_size + 20].to(DEVICE)
            forecasts[i] = f
        ...
        if model.scale_max is not None and model.scale_min is not None:
            ...
            x_adv_scaled = fake_data
            real_pred_scaled = (model.scale_max - model.scale_min) * ((real_pred + 1)/2) + model.scale_min
        else:
            real_data_scaled = real_data
            x_adv_scaled = fake_data
