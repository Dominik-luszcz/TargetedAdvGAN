...
class Generator(nn.Module):
    def __init__(self, input_dim, output_dim, sample_size):
        ...
        self.generator = nn.Sequential(
            TCN_Block(input_dim=input_dim, output_dim=64, kernel_size=3, dilation=1, padding=(3-1) * 1),
    ...
    def forward(self, condition, z):
        noise = z #torch.concatenate([condition, z], dim=-1)
...
class Discriminator(nn.Module):
    ...
    def forward(self, condition, data):
        ...
        noise = data # torch.concatenate([condition, data], dim=-1)
        noise = torch.permute(noise, (0, 2, 1))
