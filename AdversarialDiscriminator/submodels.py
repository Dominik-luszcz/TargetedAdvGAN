...
class Discriminator(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim):
        ...
        self.discriminator = nn.Sequential(
            ...
            nn.Conv1d(in_channels=input_dim, out_channels=hidden_dim, kernel_size=3, padding=3//2, stride=1),
            nn.LeakyReLU(),
            nn.Dropout(p=0.2),

            nn.BatchNorm1d(num_features=hidden_dim),
            nn.Conv1d(in_channels=hidden_dim, out_channels=hidden_dim * 2, kernel_size=5, padding=5//2, stride=1),
            nn.LeakyReLU(),
            nn.Dropout(p=0.2),

            nn.BatchNorm1d(num_features=hidden_dim * 2),
            nn.Conv1d(in_channels=hidden_dim * 2, out_channels=hidden_dim, kernel_size=5, padding=5//2, stride=1),
            nn.LeakyReLU(),
            nn.Dropout(p=0.2),

            nn.BatchNorm1d(num_features=hidden_dim),
            nn.Conv1d(in_channels=hidden_dim, out_channels=output_dim, kernel_size=3, padding=3//2, stride=1),
        )
