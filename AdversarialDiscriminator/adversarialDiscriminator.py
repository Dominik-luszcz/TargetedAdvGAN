...
class AdversarialDiscriminator(pl.LightningModule):
    def __init__(self, hidden_dim, input_dim = 1, output_dim = 1, lr = 1e-04, weight_decay = 1e-05):
        super().__init__()
        
        self.discriminator = Discriminator(input_dim=input_dim, hidden_dim=hidden_dim, output_dim=output_dim)
        self.lr = lr
    ...
    def training_step(self, batch):
        ...
        loss = nn.functional.binary_cross_entropy_with_logits(discriminator_outputs, labels)
...
class AdversarialDiscriminatorCallback(pl.Callback):
    ...
    def get_threshold(self, pred, labels):
        ...
        zero_precision, zero_recall = np.argwhere(precision == 0), np.argwhere(recall == 0)
        combined = np.intersect1d(zero_precision, zero_recall)
        precision, recall, thresholds = np.delete(precision, combined), np.delete(recall, combined), np.delete(thresholds, combined)
    ...
    def on_train_epoch_end(self, trainer, model: AdversarialDiscriminator):
        ...
        val_data, val_labels = self.validation_set[:, :-1].unsqueeze(-1), self.validation_set[:, -1].unsqueeze(-1)
        train_data, train_labels = self.training_set[:, :-1].unsqueeze(-1), self.training_set[:, -1].unsqueeze(-1)
        ...
        val_loss = nn.functional.binary_cross_entropy_with_logits(discriminator_outputs_val, val_labels)
        model.val_loss.append(val_loss)
        train_loss = nn.functional.binary_cross_entropy_with_logits(discriminator_outputs_train, train_labels)
