...
from sklearn.metrics import confusion_matrix, f1_score, accuracy_score, precision_score, matthews_corrcoef, cohen_kappa_score, recall_score
...
class StockDataset(Dataset):
    def __init__(self, stock_folder, split_file):
        ...
        train, val, test = split.item()["train"], split.item()["val"], split.item()["test"]
        ...
        self.real_train, self.fake_train = torch.from_numpy(np.concatenate(real_train)), torch.from_numpy(np.concatenate(fake_train))
        self.real_val, self.fake_val = torch.from_numpy(np.concatenate(real_val)), torch.from_numpy(np.concatenate(fake_val))
        self.real_test, self.fake_test = torch.from_numpy(np.concatenate(real_test)), torch.from_numpy(np.concatenate(fake_test))
...
def train_on_one_stocks(data_files, split_file, hidden_dim, batch_size, num_epochs, output_path):
    ...
    # Init the trainer
    trainer = pl.Trainer(devices='auto', accelerator='auto', accumulate_grad_batches=1, logger=False, callbacks=[train_callback, AdversarialDiscriminatorCallback(dataset.validation_set, dataset.training_set)], 
...
def test(model: AdversarialDiscriminator, test_set, output_path):
    ...
    specificity = np.sum((np.array(labels_np) == np.array(pred)) & (np.array(labels_np) == 0)) / np.sum(np.array(labels_np) == 0) if np.sum(np.array(labels_np) == 0) > 0 else 0
    ...
    plt.savefig(f'{output_path}/test_cm.png',
                bbox_inches='tight')
    plt.close()

    m = [{"Accuracy": acc, "Precision": precision, "Recall": recall, "Specificity": specificity,
