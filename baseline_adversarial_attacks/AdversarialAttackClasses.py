...
class AdversarialAttack(ABC):
    def __init__(
        self, projection_model, epsilon=0.5, lookback_length=100, forecast_length=20
    ):
        super().__init__()
    ...
    def feature_generation(self, adjprc: torch.Tensor):
        ...
        rolling_mean5 = torch.nn.functional.conv1d(
            adjprc.unsqueeze(0).unsqueeze(0), weight5, stride=1
        )

        rolling_mean10 = torch.nn.functional.conv1d(
            adjprc.unsqueeze(0).unsqueeze(0), weight10, stride=1
        )

        rolling_mean20 = torch.nn.functional.conv1d(
            adjprc.unsqueeze(0).unsqueeze(0), weight20, stride=1
        )

        # stdevs: Variance = E[x^2] - E[x]^2
        # so E[x] = rolling_meanX => E[x]^2 = rolling_meanX ^ 2
        rollingExp_mean5 = torch.nn.functional.conv1d(
            adjprc.unsqueeze(0).unsqueeze(0) ** 2, weight5, stride=1
        )
        rollingExp_mean10 = torch.nn.functional.conv1d(
            adjprc.unsqueeze(0).unsqueeze(0) ** 2, weight10, stride=1
        )
        rollingExp_mean20 = torch.nn.functional.conv1d(
            adjprc.unsqueeze(0).unsqueeze(0) ** 2, weight20, stride=1
        )
        ...
        rolling_stdev5 = torch.cat(
            [
                torch.full(
                    size=[4],
                    fill_value=torch.mean(rolling_stdev5.squeeze(0).squeeze(0)).item(),
                ),
                rolling_stdev5,
            ]
        )
        ...
        rolling_stdev10 = torch.cat(
            [
                torch.full(
                    size=[9],
                    fill_value=torch.mean(rolling_stdev10.squeeze(0).squeeze(0)).item(),
                ),
                rolling_stdev10,
            ]
        )
        ...
        rolling_stdev20 = torch.cat(
            [
                torch.full(
                    size=[19],
                    fill_value=torch.mean(rolling_stdev20.squeeze(0).squeeze(0)).item(),
                ),
                rolling_stdev20,
            ]
        )
        ...
        scale_ = (
            np.quantile(adjprc.detach().numpy(), q=0.75)
            - np.quantile(adjprc.detach().numpy(), q=0.25)
        ) / 2
    ...
    def call_model(self, adjprc: torch.Tensor, days):
        ...
        for i in range(
            0, len(features) - self.lookback_length - self.forecast_length + 1
    ...
    def attack(self, recording):
        ...
        for i in range(self.iterations):
            ...
            with torch.no_grad():
                ...
                perturbation = torch.clip(
                    (x_i + self.alpha * sign_grad) - adjprc, -self.epsilon, self.epsilon
                )
        ...
        mae = round(
            mean_absolute_error(
                adjprc[self.lookback_length :].detach().numpy(),
                normal_predictions.detach().numpy(),
            ),
            3,
        )
        mae_attack = round(
            mean_absolute_error(
                adjprc[self.lookback_length :].detach().numpy(),
                attack_predictions.detach().numpy(),
            ),
            3,
        )
...
class FGSM(BasicIterativeMethod):
    def __init__(self, projection_model, epsilon=0.5):
        super().__init__(
            projection_model=projection_model, iterations=1, epsilon=epsilon
        )
    ...
    def attack(self, recording):
        ...
        for i in range(self.iterations):
            ...
            with torch.no_grad():
                ...
                attack_x_i = torch.clamp(
                    attack_x_i, min=adjprc - self.epsilon, max=adjprc + self.epsilon
                )

                # Now we have to ensure rationality of post attack forcasts where g_sim is cosign similarity

                attack_similarity = nn.functional.cosine_similarity(
                    adjprc, attack_x_i, dim=0
                )
                upperbound_similarity = nn.functional.cosine_similarity(
                    adjprc, adjprc + self.epsilon, dim=0
                )
                lowerbound_similarity = nn.functional.cosine_similarity(
                    adjprc, adjprc - self.epsilon, dim=0
                )
...
class Slope_Attack(AdversarialAttack):
    def __init__(
        self, projection_model, iterations, epsilon=0.5, target_direction=1, c=5, d=2
    ):
        super().__init__(projection_model, epsilon)
    ...
    def attack(self, recording):
        ...
        for i in range(self.iterations):
            ...
            with torch.no_grad():
                ...
                attack_x_i = torch.clamp(
                    attack_x_i, adjprc - self.epsilon, adjprc + self.epsilon
                )
...
class CW_BasicSlope_Attack(AdversarialAttack):
    ...
    def attack(self, recording):
        ...
        for i in range(self.iterations):
            ...
            if self.target_direction != 0:
                target_direction = self.target_direction * -1
                loss = self.c * torch.exp(
                    target_direction * self.d * slope
                ) + torch.norm(adjprc - x_i, p=2)
            else:
...
class LS_Slope_Attack(AdversarialAttack):
    ...
    def attack(self, recording):
        ...
        for i in range(self.iterations):
            ...
            x = torch.arange(len(predictions), dtype=torch.float32).expand(
                1, len(predictions)
            )
...
class CW_Attack(AdversarialAttack):
    ...
    def attack(self, recording):
        ...
        for i in range(self.iterations):
            ...
            loss = self.c * nn.functional.l1_loss(
                attack_predictions, target
            ) + torch.norm(adjprc - x_i, p=2)
            loss = loss.float()
