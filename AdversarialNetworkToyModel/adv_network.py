...
class AdversarialNetwork(pl.LightningModule):
    def __init__(
        self,
        sample_size,
        time_series_metrics: dict,
        model,
        alpha=2,
        init_beta=0.00001,
        epoch_betas=[100, 300, 400, 450],
        beta_scale=5,
        scale_max=None,
        scale_min=None,
        plot_paths=".",
    ...
    def training_step(self, batch):
        ...
        adv_adjprc = torch.concat(
            [
                ...
                initial_price.unsqueeze(-1)
                * torch.exp(torch.cumsum(x_adv_scaled, dim=1)),
            ],
            dim=1,
        )
...
class DCGAN_Callback(pl.Callback):
    ...
    def on_train_epoch_end(self, trainer: pl.Trainer, model: AdversarialNetwork):
        ...
        s_loss = (
            (model.time_series_metrics["mean"] - fake_means) ** 2
            + (model.time_series_metrics["stdev"] - fake_stdevs) ** 2
            + (model.time_series_metrics["skew"] - fake_skew) ** 2
            + (model.time_series_metrics["kurtosis"] - fake_kurtosis) ** 2
        )
        ...
        adv_adjprc = torch.concat(
            [
                initial_price.unsqueeze(-1),
                initial_price.unsqueeze(-1)
                * torch.exp(torch.cumsum(fake_output, dim=1).squeeze(-1)),
            ],
            dim=1,
        )
        real_adjprc = torch.concat(
            [
                initial_price.unsqueeze(-1),
                initial_price.unsqueeze(-1)
                * torch.exp(torch.cumsum(real_data_scaled, dim=1)),
            ],
            dim=1,
        )
        ...
        with open(
            ...
            file.write("=" * 50 + "\n")
            file.write(
                f"Global Mean: {model.time_series_metrics["mean"]}, Fake Mean: {fake_means}\n"
            )
            file.write(
                f"Global Stdev: {model.time_series_metrics["stdev"]}, Fake Stdev: {fake_stdevs}\n"
            )
            file.write(
                f"Global skew: {model.time_series_metrics["skew"]}, Fake skew: {fake_skew}\n"
            )
            file.write(
                f"Global kurtosis: {model.time_series_metrics["kurtosis"]}, Fake kurtosis: {fake_kurtosis}\n"
            )
