...
def preprocess(data_path: str, output_path: str):
    ...
    for entry in Path(data_path).iterdir():
        if entry.suffix == ".csv":
            ...
            df["returns_rolling_mean3"] = df["returns_rolling_mean3"].fillna(
                df["returns"]
            )
            df["returns_rolling_mean5"] = df["returns"].rolling(5).mean()
            df["returns_rolling_mean5"] = df["returns_rolling_mean5"].fillna(
                df["returns"]
            )
            df["returns_rolling_mean10"] = df["returns"].rolling(10).mean()
            df["returns_rolling_mean10"] = df["returns_rolling_mean10"].fillna(
                df["returns"]
            )
            df["returns_rolling_mean20"] = df["returns"].rolling(20).mean()
            df["returns_rolling_mean20"] = df["returns_rolling_mean20"].fillna(
                df["returns"]
            )

            # For the NaN stdevs, fill them in with the 0
            df["returns_rolling_stdev3"] = df["returns"].rolling(3).std()
            df["returns_rolling_stdev3"] = df["returns_rolling_stdev3"].fillna(
                df["returns_rolling_stdev3"].mean()
            )
            df["returns_rolling_stdev5"] = df["returns"].rolling(5).std()
            df["returns_rolling_stdev5"] = df["returns_rolling_stdev5"].fillna(
                df["returns_rolling_stdev5"].mean()
            )
            df["returns_rolling_stdev10"] = df["returns"].rolling(10).std()
            df["returns_rolling_stdev10"] = df["returns_rolling_stdev10"].fillna(
                df["returns_rolling_stdev10"].mean()
            )
            df["returns_rolling_stdev20"] = df["returns"].rolling(20).std()
            df["returns_rolling_stdev20"] = df["returns_rolling_stdev20"].fillna(
                df["returns_rolling_stdev20"].mean()
            )
            ...
            to_scale = [
                "returns_rolling_mean3",
                "returns_rolling_mean5",
                "returns_rolling_mean10",
                "returns_rolling_mean20",
                "returns_rolling_stdev3",
                "returns_rolling_stdev5",
                "returns_rolling_stdev10",
                "returns_rolling_stdev20",
            ]
            df[to_scale] = standardScalar.fit_transform(df[to_scale])
            df.to_csv(
                f"{output_path}/{df["ticker"].to_numpy()[0]}-features.csv", index=False
            )
