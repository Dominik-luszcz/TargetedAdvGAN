...
def get_CRSP_ret_data_with_dates(
    db: wrds.Connection,
    ticker_symbol: str,
    start_date: str,
    end_date: str,
    ...
    df = db.raw_sql(
        f"""
                    SELECT permno, '{ticker_symbol}' AS ticker, ret as returns, date AS date
                    FROM crsp.dsf
                    WHERE permno = '{desired_permno}'
                        AND date BETWEEN '{start_date}' AND '{end_date}'"""
    )
