...
def determine_split(ticker_path: str, feature_path: str) -> None:
    ...
    train_stocks, test_val_stocks = train_test_split(
        filtered_data, train_size=0.75, stratify=filtered_data["label"], random_state=23
    )
    ...
    print(
        f"Train size: {len(train_stocks)}, Test Size: {len(test_stocks)}, Val Size: {len(val_stocks)}"
    )
    ...
    np.save(
        "training_split.npy",
        {
            "train": train_stocks["Symbol"].to_numpy(),
