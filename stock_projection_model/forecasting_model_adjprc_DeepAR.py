...
def find_optimal_learning_rate(
    trainer: pl.Trainer, model: DeepAR, train_dataloader, val_dataloader
):
...
if __name__ == "__main__":
    ...
    model_state_dict = torch.load("DeepAR_forecasting_model.pt")
    # model = DeepAR.from_dataset(train_dataset, weight_decay=1e-3,
    #                            hidden_size=64, log_val_interval=1, batch_normalization=True,
    #                            loss=QuantileLoss(quantiles=[0.001, 0.01, 0.05, 0.5, 0.95, 0.99, 0.999]))

    params = torch.load("./DeepAR_params.pt", weights_only=False)
    params["loss"] = QuantileLoss(quantiles=[0.001, 0.01, 0.05, 0.5, 0.95, 0.99, 0.999])
    model = DeepAR(**params)
