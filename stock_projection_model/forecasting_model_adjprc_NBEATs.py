...
sys.path.append("TargetedAdvGAN")
...
def set_up_dataset(feature_path: str, split_file: str):
    ...
    train_datset = TimeSeriesDataSet(
        ...
        time_varying_known_reals=[],     # <- add this
        static_reals=[],                 # <- add this
        static_categoricals=[],          # <- add this
        time_varying_known_categoricals=[],  # <- add
...
def train(
    ...
    early_stop_callback = EarlyStopping(
        monitor="val_loss", min_delta=1e-5, patience=15, verbose=False, mode="min"
    )
...
def test_last_window(
    ...
    with PdfPages(pdf_path) as pdf:
        for stock, data in stock_projection.items():
            ...
            plt.title(
                f"Predicted vs Actual for stock {stock} (last window), MAE: {mase}"
            )
            plt.legend()
            plt.xlabel("Day")
            plt.ylabel("adjprc")
...
def test(
    ...
    ends = torch.cat(
        [(starts)[1:, :], torch.tensor([len(time_idx)]).unsqueeze(-1)], dim=0
    )
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        for start, end in starts_and_ends:
            ...
            df = df.groupby("time").agg({"Prediction": "mean", "Actual": "mean"})
            ...
            plt.title(
                f"Predicted vs Actual for stock {tickers[ticker_index]} (all), MAE: {mae}"
            )
            plt.legend()
            plt.xlabel("Day")
            plt.ylabel("adjprc")
...
def find_optimal_learning_rate(
    trainer: pl.Trainer, model: NBeats, train_dataloader, val_dataloader
):
...
if __name__ == "__main__":
    ...
    # Set up the datasets
    train_dataset, val_dataset, test_dataset = set_up_dataset(
        "SP500_Features_adjprc", "training_split.npy"
    )

    train_dataset.save("train_dataset_nbeats.npy")

    # Set up the dataloaders
    train_loader = train_dataset.to_dataloader(
        train=True, batch_size=500, num_workers=19, persistent_workers=True
    )
    val_loader = val_dataset.to_dataloader(
        train=False, batch_size=500, num_workers=19, persistent_workers=True
    )
    ...
    model_state_dict = torch.load("NBEATS_forecasting_model.pt")
    # model = NBeats.from_dataset(train_dataset, weight_decay=1e-3,
    #                            hidden_size=64, log_val_interval=1, batch_normalization=True,
    #                            loss=QuantileLoss(quantiles=[0.001, 0.01, 0.05, 0.5, 0.95, 0.99, 0.999]))

    params = torch.load("./NBEATS_params.pt", weights_only=False)
    params["loss"] = QuantileLoss(quantiles=[0.001, 0.01, 0.05, 0.5, 0.95, 0.99, 0.999])
    model = NBeats(**params)
    ...
    test(model, test_loader, test_dataset, mode="test", output_path="Forecast_outputs_NBEATS")
    t2 = datetime.now()
