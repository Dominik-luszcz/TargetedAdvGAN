...
def train(
    ...
    model = LSTM(
        input_size=train_dataset.size()[1],
        weight_decay=1e-4,
        hidden_size=64,
        num_layers=3,
        batch_first=True
...
def find_optimal_learning_rate(
    trainer: pl.Trainer, model: LSTM, train_dataloader, val_dataloader
):
...
if __name__ == "__main__":
    ...
    model_state_dict = torch.load("LSTM_forecasting_model.pt")
    # model = LSTM.from_dataset(train_dataset, weight_decay=1e-3,
    #                            hidden_size=64, log_val_interval=1, batch_normalization=True,
    #                            loss=QuantileLoss(quantiles=[0.001, 0.01, 0.05, 0.5, 0.95, 0.99, 0.999]))

    params = torch.load("./LSTM_params.pt", weights_only=False)
    params["loss"] = QuantileLoss(quantiles=[0.001, 0.01, 0.05, 0.5, 0.95, 0.99, 0.999])
    model = LSTM(**params)
